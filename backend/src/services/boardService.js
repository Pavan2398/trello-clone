...
class BoardService {
  async createBoard(title, background_color, owner_id, background_image, organization_id) {
    ...
    if (newBoard) {
      ...
      // Optimized: Batch insert labels in one query
      const labelSafeValues = [];
      const labelPlaceholders = defaultLabels.map((label, i) => {
        const offset = i * 3;
        labelSafeValues.push(newBoard.id, label.name, label.color);
      ...
      // Wait for labels to be created (important part of board setup)
      await db.query(labelQuery, labelSafeValues);

      // Optimized: Non-blocking audit log (fire and forget)
      auditService.logAction(newBoard.id, owner_id, 'board', newBoard.id, 'create', { title })
  ...
  async getBoardById(boardId, userId = 1) {
    ...
    // 4. Fetch Meta (Labels & Members & Checklists)
    if (cards.length > 0) {
      ...
      // Checklists
      const checklistsQuery = `SELECT * FROM checklists WHERE card_id = ANY($1) ORDER BY position ASC, created_at ASC`;
      const checklistsResult = await db.query(checklistsQuery, [cardIds]);
      const allChecklists = checklistsResult.rows;

      // Checklist Items
      let allChecklistItems = [];
      if (allChecklists.length > 0) {
        const checklistIds = allChecklists.map(c => c.id);
        const itemsQuery = `SELECT * FROM checklist_items WHERE checklist_id = ANY($1) ORDER BY position ASC, created_at ASC`;
        const itemsResult = await db.query(itemsQuery, [checklistIds]);
        allChecklistItems = itemsResult.rows;
      }
      ...
      cards = cards.map(card => {
        ...
        const cardChecklists = allChecklists
          ...
          .map(cl => ({
            ...
            items: allChecklistItems
              ...
              .map(item => ({
                ...item,
                completed: item.is_completed, // Normalize for frontend
                text: item.content // Normalize for frontend
              }))
          }));

        return { ...card, labels: cardLabels, members: cardMembers, checklists: cardChecklists };
      });
    }
