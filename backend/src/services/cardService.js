...
class CardService {
  ...
  async updateCard(id, updates, userId = 1) {
    const { title, description, due_date, list_id, position, is_due_complete } = updates;
    const fields = [];
    const values = [];
    let idx = 1;
    ...
    if (is_due_complete !== undefined) {
      fields.push(`is_due_complete = $${idx++}`);
      values.push(is_due_complete);
    }
