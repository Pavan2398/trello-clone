                            ...
                            {editedCard.due_date && (
                                <div className="space-y-1.5">
                                    ...
                                    <div className="flex items-center gap-2">
                                        <Checkbox
                                            id="due"
                                            checked={!!editedCard.is_due_complete}
                                            onCheckedChange={checked => handleUpdate({ is_due_complete: checked })}
                                            className="data-[state=checked]:bg-indigo-600 border-slate-300"
                                        />
                                        <label htmlFor="due" className="text-sm font-semibold text-slate-700 select-none cursor-pointer">
                                            Due date {editedCard.is_due_complete ? <span className="bg-green-100 text-green-700 px-1.5 py-0.5 rounded text-[10px] uppercase font-bold ml-1 tracking-wide">Complete</span> : ''}
                                        </label>
                                        <span className={cn(
                                            "text-xs font-normal ml-auto px-1.5 py-0.5 rounded",
                                            editedCard.is_due_complete ? "bg-green-50 text-green-600" : (new Date(editedCard.due_date) < new Date() ? "bg-red-50 text-red-600 font-bold" : "bg-slate-100 text-slate-500")
                                        )}>
                                            {format(new Date(editedCard.due_date), 'MMM dd at h:mm a')}
                                        </span>
                                    </div>
                                </div>
                            )}
                            ...
                            <Popover>
                                <PopoverTrigger asChild>
                                    ...
                                    >
                                        ...
                                        {editedCard.due_date ? format(new Date(editedCard.due_date), 'MMM dd, yyyy') : 'Dates'}
                                    </Button>
                                </PopoverTrigger>
